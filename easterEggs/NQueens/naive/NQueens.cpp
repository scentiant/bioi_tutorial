...
// solutions, it enables a branch-and-bound approach to probing the
// solution space. (This is in contrast to enumerating and testing
// every potential (complete) puzzle solution.)
...
void NQueens::recurse_over_solns(const N_t& row,const N_t& N) {
  for(auto i{0};i<N;board[IND(row,i++,N)]=false) {
    ...
    // We could save time by caching results from testing the
    // feasibility of prior board configurations, but I don't
    // want to "clutter" this example with code for memoization,
    // use of coroutines, etc. We're more after demonstrating the
    // general "shape" of applying a filter to prune search efforts.
...
// problem in greater detail (e.g., symmetry of solutions).
// What we're after here is more to demonstrate the generality
// achievable with using a recursive solution.
void NQueens::enumerate_NQueens_solutions(void)
{
  N_t& N=side_length; // a reference to save typing!
  try {
    board=new bool[N*N];
  }
  catch(bad_alloc&) {
...
*/
  ...
  // a recursive solution to the problem becomes apparent.
